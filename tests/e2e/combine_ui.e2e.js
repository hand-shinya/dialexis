...
(async () => {
  ...
  // 語ノード(root)を実マウスでクリック→メニュー。canvasの内部座標を再計算し、
  // Gがまだ薄い/取得中なら例外で止めず、結果をFAILとして記録する。
  await page.locator("#origin-graph").scrollIntoViewIfNeeded();
  const pt = await page.evaluate(() => __dx && __dx.nodeClientXY(n => n.kind === "word"));
  if (pt) await page.mouse.click(pt.x, pt.y);
  await page.waitForTimeout(600);
  ok("語ノードのメニューが出る", !!(await page.$("#graph-menu")));
  const combineItem = page.locator("#graph-menu .gm-item").filter({ hasText: "組み合わせ" }).first();
  if (await combineItem.count()) await combineItem.click();
  await page.waitForTimeout(600);
  const panel = await page.$("#cmb-b");
  ok("『別の語と組み合わせる』でパネル＋入力が出る", !!panel);
  if (panel) {
    await page.fill("#cmb-b", "労働");
    const andButton = page.locator(".cmb-op").filter({ hasText: "AND" }).first();
    if (await andButton.count()) await andButton.click();
