...
|---|---|---|
| Source 資料層 | connectors（ライブ参照）＋ provenance。**オリエンテーションは主題の知の伝統でルーティング**（西洋=SEP／日本語圏=NDL・CiNii） | TEI/IIIF接続、PhilPapers、非西洋（中国/インド/イスラーム）情報源 |
...
現行コネクタ：`wikidata` / `wikipedia` / `sep`（英語圏オリエンテーション・関連性ゲート付き）/ `openalex` / `crossref` / `opencitations` / `gutendex` / `ndl`（国立国会図書館サーチ＝邦訳＋本人/関連書）/ `cinii`（CiNii Research＝日本語圏の論文・書籍）。`orientation_plan()`（`app/main.py`）が主題の知の伝統でオリエンテーション源を選ぶ（西洋=SEP／日本語圏=NDL・CiNii。将来 zh/hi/ar は分岐追加で拡張）。

すべてのコネクタは `connectors/base.py` の `cached_get_json()`（XML源の NDL は同等の `_get_xml()`）を使い、次の統一封筒で返す：
